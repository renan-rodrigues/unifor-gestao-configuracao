...
public class SmsReceiverController extends AbstractController {
	...
	public SmsReceiverController(MainUIMidlet midlet, AlbumData albumData, AlbumListScreen albumListScreen) {
 	...
 	/**
	 * Handle SMS specific events. If we are given a standard command that is
	 * handled by the BaseController, pass the handling off to our super class
	...
	public boolean handleCommand(Command c) {
		...
		if (label.equals("Accept Photo")) {
			...
			// [NC] Changed in the scenario 07: just the first line below to
			// support generic AbstractController
